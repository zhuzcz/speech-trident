...
| Date    | Model Name     | Paper Title                                                                                                           | Link                                      | 简介                             |
| ------- | -------------- | --------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | ----------------------------------------- |
| 2024-12 |   CosyVoice 2  | CosyVoice 2: Scalable Streaming Speech Synthesis with Large Language Models | [Paper](https://arxiv.org/pdf/2412.10117) | text bpe输入，speech token FSQ量化，LLM初始化，flowmatching生成mel，去除了speaker embedding，flow中依然使用 |
...
| 2024-11 | hertz-dev    | [blog](https://si.inc/hertz-dev/)  | [GitHub](https://github.com/Standard-Intelligence/hertz-dev) | VAE建模音频，其中enc得到的均值方差采样后送入模型，lm后接了FSQ量化，然后继续通过网络输出下一帧的连续向量。训练方式可能对第一步的VAE再次VQVAE，也有可能直接KL训练下一帧的预测 |
...
| 2024-08 | WavTokenizer             | WavTokenizer: an Efficient Acoustic Discrete Codec Tokenizer for Audio Language Modeling | [paper](https://arxiv.org/abs/2408.16532) | VQ-GAN，单个VQ，多尺度判别器，dec不采用镜像结构，vocoder逆傅里叶上采样 |
...
| 2024-04 | SemantiCodec             | SemantiCodec: An Ultra Low Bitrate Semantic Audio Codec for General Sound                                                                              | [paper](https://arxiv.org/abs/2405.00233) | 类似SpeechTokenizer，分为semantic和acoustic，未开源，不用看 |
...
| 2024-03 | FACodec                  | NaturalSpeech 3: Zero-Shot Speech Synthesis with Factorized Codec and Diffusion Models                                                                 | [paper](https://arxiv.org/abs/2403.03100) | 与之前都不同。将enc out解耦了4个部分，prosody, content, acoustic, timbre，前三个离散，timbre作为global representations，有inference代码 |
...
| 2023-09 | FunCodec             | FunCodec: A Fundamental, Reproducible and Integrable Open-source Toolkit for Neural Speech Codec                                                           | [paper](https://arxiv.org/abs/2309.07405) | 主打开源 |
| 2023-08 | SpeechTokenizer      | Speechtokenizer: Unified speech tokenizer for speech large language models                                                                                 | [paper](https://arxiv.org/abs/2308.16692) | 强行约束第一层的VQ layer学习audio中的语义信息（Hubert），其他层学习跟[acoustic](https://zhida.zhihu.com/search?content_id=242893191&content_type=Article&match_order=1&q=acoustic&zhida_source=entity)相关的信息，有infer |
...
| 2023-05 | AudioDec             | Audiodec: An open-source streaming highfidelity neural audio codec                                                                                         | [paper](https://arxiv.org/abs/2305.16608) | Encodec同期工作，Enc-vq-Vocoder，enc与SoundStream一致，loss仅用mel loss，Encodec使用了multi-scale STFT loss和wavform loss |
| 2023-05 | HiFi-Codec           | Hifi-codec: Group-residual vector quantization for high fidelity audio codec                                                                               | [paper](https://arxiv.org/abs/2305.02765) | SoundStream上，引入了组rvq，lantet fea分为了2组 |
...
| 2022-10 | EnCodec              | High fidelity neural audio compression                                                                                                                     | [paper](https://arxiv.org/abs/2210.13438) | 沿用SoundStream，1. 通过GAN加入multi-scale STFT loss; 2. RVQ基础上加了轻量transformer，压缩codebook，有infer |
...
| 2021-07 | SoundStream          | SoundStream: An End-to-End Neural Audio Codec                                                                                                              | [paper](https://arxiv.org/abs/2107.03312) | 奠定了audio codec的原型：Encoder -> RVQ Quantizer -> Decoder; enc 使用SEANet, dec 镜像对称 |
